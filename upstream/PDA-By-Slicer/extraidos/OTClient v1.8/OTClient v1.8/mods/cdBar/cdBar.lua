...
function cdBar.barChange(but, num, lvl, lvlPoke)   --alterado v2.8                    
if not g_game.isOnline() then return end
...
if num and num >= 1 then
   ...
   if botoes['Button' ..but] then 
     ...
     if button:getTooltip() then
        ...
        elseif tonumber(lvl) ~= nil and player:getLevel() < lvl then        --alterado v2.8
           button:setImageSource(pathOff) 
           button:setText(lvl)
           button:setColor('#FF0000')
        else
...
for i = 1, #table do
    if tonumber(table[i][3]) ~= nil then
       cdBar.barChange(i, tonumber(table[i][1]), tonumber(table[i][2]), tonumber(table[i][3])) --alterado v2.8
    else
