...
function onGameEditText(id, itemId, maxLength, text, writter, time)
  ...
  --------------------------------------------------------
  local pokedex = false
  if string.find(text, "Ability:") and string.find(text, "Evolutions:") then
     pokedex = true
  end
  --------------------------------------------------------
  local text2 = ""
  local count = 0
  if #text > 0 and string.find(text, "\n") then   --alterado limita o tamanho do texto pro lado >>
     local t1 = text:explode("\n")
     for i = 1, #t1 do
         local t2 = t1[i]:explode(" ")
         for a = 1, #t2 do
             if (#t2[a]+1) + count > 33 then
                t2[a] = "\n"..t2[a]
                count = 0
             end
             text2 = text2..""..t2[a].." "
