BUY = 1
SELL = 2
CURRENCY = 'dollars'   --alterado v1.8
LAST_INVENTORY = 10
...
function onQuantityValueChange(quantity)
  if quantityLabel and selectedItem then
    ...
    priceLabel:setText( (math.floor(tonumber(getItemPrice(selectedItem))/100)) .. ' ' .. CURRENCY)  --alterado
  end
end
...
function canTradeItem(item)
  if getCurrentTradeType() == BUY then
    return playerMoney >= getItemPrice(item)
  else
...
function refreshItem(item)
  ...
  priceLabel:setText( (math.floor(tonumber(getItemPrice(item))/100)) .. ' ' .. CURRENCY) --alterado
...
function refreshTradeItems()
  ...
  for key,item in pairs(currentTradeItems) do
    ...
    text = text .. name
    local priceTest = tonumber(item.price)/100
    local price = (priceTest < 1 and priceTest or math.floor(priceTest)) .. ' ' .. CURRENCY
    text = text .. '\n' .. price
...
function refreshPlayerGoods()
  moneyLabel:setText( (math.floor(tonumber(playerMoney))) .. ' ' .. CURRENCY)
