...
start_wine() {
  ...
  if ! pid_alive "$PIDS/server.pid"; then
    say "Iniciando servidor PDA 2.9 em 127.0.0.1:7171/7172..."
    (
      cd "$SERVER_DIR"
...
run_local() {
  ...
  printf 'Servidor: 127.0.0.1:7171 / jogo: 7172 / protocolo: 8.54\n'
...
status() {
  ...
  for name in xvfb x11vnc novnc server client; do
...
Uso:
  ...
  bash poketag.sh install  instala Wine + Xvfb + noVNC no Cloud Shell
  bash poketag.sh run      prepara e inicia servidor + cliente local
  bash poketag.sh status   mostra processos e portas
